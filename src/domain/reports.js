...
async function revenueForClient(pool, clientId, { from, to } = {}) {
  const { rows } = await pool.query(
    `SELECT
     ...
     FROM invoices
     WHERE client_id = $1 AND status = 'PAID' AND is_test = FALSE
  ...
  const { rows } = await pool.query(
    `SELECT c.id, c.name,
     ...
     LEFT JOIN invoices i
       ON i.client_id = c.id AND i.status = 'PAID' AND i.is_test = FALSE
...
async function statusBreakdown(pool, clientId = null) {
  const { rows } = await pool.query(
    `SELECT status, count(*) AS n FROM invoices
     WHERE is_test = FALSE AND ($1::text IS NULL OR client_id = $1)
